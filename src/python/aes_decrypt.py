...
def InvShiftRows(state):
    ...
    # S(r,(c+shift(r,Nb))mod Nb)' = S(r,c) for0<r<4 and 0≤c<Nb (5.8)
    # shift(1,4)=1; shift(2,4)=2; shift(3,4)=3. (5.4)
    # the row is the same by how much you rotate
    for r in range(4):
        for c in range(4):
            state_shift[r][(c + r) % Nb] = state[r][c]
    state = state_shift
    logger.debug("InvShiftRows")
    print_state(state)
    return state
...
def InvMixColumns(state):
    ...
    for c in range(4):
        state_mix[0][c] = mult(0x0e, state[0][c]) ^  mult(0x0b, state[1][c]) ^ mult(0x0d, state[2][c]) ^ mult(0x09, state[3][c])
        state_mix[1][c] = mult(0x09, state[0][c]) ^  mult(0x0e, state[1][c]) ^ mult(0x0b, state[2][c]) ^ mult(0x0d, state[3][c])
        state_mix[2][c] = mult(0x0d, state[0][c]) ^  mult(0x09, state[1][c]) ^ mult(0x0e, state[2][c]) ^ mult(0x0b, state[3][c])
        state_mix[3][c] = mult(0x0b, state[0][c]) ^  mult(0x0d, state[1][c]) ^ mult(0x09, state[2][c]) ^ mult(0x0e, state[3][c])
